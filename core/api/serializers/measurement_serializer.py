...
class MeasurementGroupedPeriodeSerializer(serializers.Serializer):
    ...
    gas_abbreviation = serializers.CharField(
        source='sensor__gas_type__abbreviation')
...
class MeasurementPaginationSerializer(serializers.ModelSerializer):
    device_name = serializers.CharField(
        source='sensor.device.name', read_only=True)
    gas_abbreviation = serializers.CharField(
        source='sensor.gas_type.abbreviation', read_only=True)
...
class CreateMeasurementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Measurement
        fields = ['deviceId', 'date', 'time', 'pm25',
                  'pm10', 'co', 'no2', 'so2', 'temperature']
